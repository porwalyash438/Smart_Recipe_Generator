...
function searchRecipes() {
  ...
  setTimeout(() => {
    ...
    const filtered = recipes.filter(r =>
      ingredients.every(ing => r.ingredients.join(" ").toLowerCase().includes(ing))
    );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("recipes.json")
    ...
    .then(data => {
      ...
      data.forEach(r => r.ingredients.forEach(i => allIngredients.add(i.toLowerCase())));
  ...
  searchBtn.addEventListener("click", () => {
    const selected = [...document.querySelectorAll("#ingredient-list input:checked")].map(cb => cb.value);
    ...
    const matches = recipes.filter(recipe =>
      selected.every(sel => recipe.ingredients.map(i => i.toLowerCase()).includes(sel))
    );
  ...
  function displayResults(list) {
    resultsEl.innerHTML = "";
    if (list.length === 0) {
      resultsEl.innerHTML = "<p>No recipes found with selected ingredients.</p>";
      return;
    }
